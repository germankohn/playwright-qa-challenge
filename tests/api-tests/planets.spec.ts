...
test.describe('Planet API Tests', () => {   

    test('GET /planets - Happy Path', async ({ request }) => {
        // Registrar el tiempo de inicio
        const startTime = Date.now();
        // Hacer la solicitud GET a la API de planets
    ...
    test('GET /planets - Validar JSON schema', async ({ request }) => {
        // Hacer la solicitud GET a la API de planets
        const response = await request.get(`${API_BASE_URL}/planets`);
        // Obtener el cuerpo de la respuesta en formato JSON
        const body = await response.json();
    ...
    test('GET /planets/0 - No encontrado (404)', async ({ request }) => {
        // Hacer la solicitud GET a un planet inexistente
